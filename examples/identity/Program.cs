...
using Allus.IdentityExample;
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.FileProviders;
using Microsoft.Extensions.Logging;

// One-command launcher for the identity example (contract §"Contract versioning", README).
//
//   dotnet run
//
// Steps:
...
app.MapPost("/api/scenarios/{id:int}/config", (HttpContext ctx, int id) => server.SaveConfig(ctx, id));
app.MapPost("/api/scenarios/{id:int}/start", (HttpContext ctx, int id) => server.Start(ctx, id));
app.MapPost("/api/scenarios/{id:int}/enroll", (HttpContext ctx, int id) => server.Enroll(ctx, id));
app.MapPost("/api/scenarios/{id:int}/clear", async (HttpContext ctx, int id) =>
{
    rt.ClearScenario(id);
    await WriteOk(ctx);
});
...
app.MapGet("/callback", (HttpContext ctx) => server.Callback(ctx));
