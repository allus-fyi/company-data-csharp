...
* **1-on-1** — `SendMessageAsync(connectionId, text)`. **End-to-end encrypted**: the
  SDK encrypts one copy to the person's public key and one to your service key
  before anything leaves the process, so the person reads it in their app and you can
  re-read your own outbound text. The platform stores ciphertext only.
* **Broadcast** — `BroadcastMessageAsync(text)`. One **plaintext** message to every
...
`SendMessageAsync` answers **201** with the created message carrying `message_id`, and
...
async Task Handle(Change change)
{
    ...
    Console.WriteLine($"{change.PersonId}: {change.MessageBody}");   // already decrypted for you

    // Reply on the same connection. PersonPublicKey rides the event, so no second
    // key lookup is needed.
    await client.SendMessageAsync(change.ConnectionId!, "Thanks — we're on it.",
    ...
    await client.MarkMessagesReadAsync(change.ConnectionId!, change.MessageId!);
}

await client.ProcessChangesAsync(Handle);
```

`MarkMessagesReadAsync` is bounded by the boundary message: a message that arrived
while you were working is **not** swept, and a repeat is a no-op. The boundary must be
a message the person sent on that connection — anything else is refused with an
`ApiException` carrying `company_data.ack_boundary_invalid` (400).

```csharp
// One plaintext announcement to everyone connected to the service.
await client.BroadcastMessageAsync("We're closed on Friday.");
```

Refusals surface as `ApiException` carrying the platform `error_key`:
...
`RateLimitException`/`ApiException`.
...
`2fa_enroll` (opt a person into 2FA — see below). `PollResultAsync(state)` drives the detached mode.
...
// SAME challenge and sends no second push. context is plain text shown on the person's card (null for none).
var ch = await client.TwoFactor.ChallengeAsync("2I6UF3",
    idempotencyKey: "login-8f3c1a", context: "Sign-in from Chrome");
if (ch.MatchingDigits is not null)                 // number matching is on for this service
    ShowOnLoginPage(ch.MatchingDigits);            // the person types these back into the app; the server checks them

// Wait for the terminal outcome — polls ResultAsync for you (defaults: 600s timeout, 2s interval),
// throws ApiException on timeout.
var res = await client.TwoFactor.WaitForResultAsync(ch.ChallengeId); // or ResultAsync(ch.ChallengeId) to poll once yourself
...
- **Burn-on-read.** The first read of a terminal state (`approved` | `denied` | `expired` | `revoked`)
  ...
  `WaitForResultAsync` returns that first terminal read and never re-reads a consumed challenge.
...
- **Enrollment.** Only an enrolled person can be challenged (an un-enrolled `share_code` is `404`).
  ...
  `responseMode: "detached"` + `PollResultAsync(state)`, which returns `{"enrolled": true, "state": ...}`
